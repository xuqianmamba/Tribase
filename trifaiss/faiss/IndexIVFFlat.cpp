...
struct IVFFlatScanner : InvertedListScanner {
    ...
    size_t my_scan_codes(
        ...
        const float* list_vecs = (const float*)codes;
        size_t nup = 0;
        float max_radius = distances2center[list_size - 1];
        float diff_cos, diff_sin;
        float max_radius_plus_coarse_distance = max_radius + coarse_distance;
        float inv_two_times_sqrt_max_radius_times_coarse_distance = 1 / (2 * sqrt(max_radius * coarse_distance));
        float inv_sqrt_coarse_distance = 1 / sqrt(coarse_distance);
        float point5_times_inv_sqrt_coarse_distance = 0.5 * inv_sqrt_coarse_distance;
        ...
        for (size_t j = 0; j < list_size; j++) {
            ...
            if (C::cmp(simi[0], dis)) {
                ...
                if (nearest_cos_dis){
                    if (max_radius + simi[0] >= coarse_distance) {
                        // diff_cos = sqrt(1 - simi[0] / coarse_distance);
                        diff_cos = sqrt(coarse_distance - simi[0]) * inv_sqrt_coarse_distance;
                    } else {
                        diff_cos = (max_radius_plus_coarse_distance - simi[0]) * inv_two_times_sqrt_max_radius_times_coarse_distance;
                    }
            ...
            if (nearest_cos_dis && coarse_distance > simi[0]) {
...
#endif
                    for (size_t skip_fake_id = (j + skip_count) * sub_k; skip_fake_id < (j + skip_count) * sub_k + sub_k; skip_fake_id++) {
                        size_t skip_true_id = nearest_cos_id[skip_fake_id];
                        if (skip_true_id > 0 && nearest_cos_dis[skip_fake_id] > cut_degree_cos_minus) {
                            if (skip_true_id > skip_count) {
#ifdef DEBUG
                                if (if_compute[skip_true_id - skip_count] && j < skip_true_id - skip_count) {
                                    compute_skip_cos_true++;
                                }
#endif
                                if_compute[skip_true_id - skip_count] = false;
                            }
                        } else {
#ifdef DEBUG
                            used_sub_k_cos += skip_fake_id - (j + skip_count) * sub_k;
#endif
                            break;
                        }
                    }
                    for (size_t skip_fake_id = (list_size + skip_count + j + skip_count) * sub_k; skip_fake_id < (list_size + skip_count + j + skip_count) * sub_k + sub_k; skip_fake_id++) {
                        size_t skip_true_id = nearest_cos_id[skip_fake_id];
...
#ifdef DEBUG
                            used_sub_k_cos += skip_fake_id - (list_size + skip_count + j + skip_count) * sub_k;
#endif
                            break;
                        }
                    }
            ...
            if (nearest_L2_dis) {
                if (dis > simi[0]) {
                    used_sub_k_L2_count++;
                    for (size_t skip_fake_id = (j + skip_count) * sub_k; skip_fake_id < (j + skip_count) * sub_k + sub_k; skip_fake_id++) {
                        size_t tmp = nearest_L2_dis[skip_fake_id] + sqrt_simi;
                        if (nearest_L2_id[skip_fake_id] > 0 && dis > tmp * tmp) {  // already sqrt nearest_L2_dis
...
#ifdef DEBUG
                            used_sub_k_L2 += skip_fake_id - (j + skip_count) * sub_k;
#endif
                            break;
                        }
                    }
