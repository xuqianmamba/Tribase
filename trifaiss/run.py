...
def output_result(result_path, I, D):
    if not osp.exists(osp.dirname(result_path)):
        os.makedirs(osp.dirname(result_path))
...
def test_main(
    ...
    index = faiss.IndexIVFwithDistance(
        quantizer, d, nlist, faiss.METRIC_L2, sub_k, sub_nlist, sub_nprobe, sub_sample
    )
    ...
    os.environ["SKIP_LOGGING_PATH"] = osp.join(root, "skip.json")

    bar = tqdm(total=len(k_list) * len(opt_levels) * sum(nprobe_list))
    for k in k_list:
        groundtruth_generate_time = None
        max_nprobe = max(nprobe_list)
        groundtruth_path = osp.join(
            root, "benchmarks", dataset, "result", f"groundtruth_{k}.txt"
        )
        ...
        for opt_level in opt_levels:
            ...
            for nprobe in nprobe_list:
                ...
                #     continue
                # if simi_ratio > 1 and nprobe == nlist:
                #     yield dataset, k, nlist, nprobe, simi_ratio, 1, 0, xq.shape[
                #         0
                #     ] * xb.shape[0], 1
                #     continue
                if not (
...
def main():
    ...
    LOG_PATH = osp.join(root, "log2.txt")
    if False:
